...
pie(table(list_mad$room_type), labels = piepercent_mad, main = "Madrid pie chart", col = rainbow(length(table(list_mad$room_type))))
legend("topright", c("Entire home/apt","Hotel room","Private room","Shared room"), cex = 0.8,
       fill = rainbow(length(table(list_mad$room_type))))
```
```{r, echo=FALSE}
# Basic piechart
piepercent_par<- round(100*table(list_par$room_type)/sum(table(list_par$room_type)), 1)

pie(table(list_par$room_type), labels = piepercent_par, main = "Paris pie chart", col = rainbow(length(table(list_par$room_type))))
legend("topright", c("Entire home/apt","Hotel room","Private room","Shared room"), cex = 0.8,
       fill = rainbow(length(table(list_par$room_type))))
```
...
pie(table(list_rom$room_type), labels = piepercent_rom, main = "Rome pie chart", col = rainbow(length(table(list_rom$room_type))))
legend("topright", c("Entire home/apt","Hotel room","Private room","Shared room"), cex = 0.8,
       fill = rainbow(length(table(list_rom$room_type))))
```
**Optie 1**
...
ggplot(total_list, aes(x = room_type)) +
    ...
    scale_fill_viridis_d()
```
**Niet goed nog, krijg hem niet in percentages**
