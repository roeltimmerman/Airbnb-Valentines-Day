...
## Prices
The variables used in this analysis are a metric variable 'price' and a dummy variable 'valentinesday', which takes the value of 0 if the booking date is not Valentine's Day, and 1 if the date is Valentine's Day. A histogram of the frequencies per total price of a listing is presented below. The average price of an Airbnb listing from the full data is 'r mean_price'. The mean of the 'valentinesday' dummy variable is 'r mean_valentinesday'. There are 29804 bookings recorded on Valentine's Day, and 801227 when it is not Valentine's Day. This means that the number of observations on Valentine's Day is approximately 3.7%, which can be seen in the summary statistics below. 
...
We compare the price on Valentine's Day with the price on other days in February that can be seen in the boxplot below. What we can conclude from the boxplot is that there is minimal to no difference between the averages of the price on Valentine's Day and other days in February. Looking at the average price per city, we can conclude that the average for Paris is the highest among all cities. Furthermore, Madrid has the lowest average price. This is verified using the lapply function of the price per cities. 
...
## Bookings
The dummy variable booked consists of two categories; not booked (0) and booked (1). By looking at the average 'r mean_booked' in the dataset, we can conclude that Airbnb's are more booked than not booked in the chosen time period  of this dataset (February 2021). This conclusion can be verified by printing out the table and the histogram for the variable 'booked'. On Valentine's Day, 53657 Airbnbs were booked according to the data for the cities Rome, Madrid and Paris. If we compare this with a random date in February, we can see that there is a slight difference between the two dates. It is interesting to see that 53657 Airbnbs were booked on Valentine's Day  compared to the randomly generated date where 53826 Airbnbs were booked. We can conclude that no more Airbnbs were booked in Rome, MAdrdid and Paris on Valentine's Day than on any other randomly generated date in February. 
...
This analysis will answer the following question: "Is there a significant difference in the number of bookings on Airbnb on Valentine's day? If so, is this difference also greater in a city with a larger population?". To do this, a logistic regression will be performed. 
